 ...
 */
template <typename Pred, typename T>
double kspace(T&& params, Pred predicate, int iterations, double abs){
	double integral = 0.;
	double tmp;
//integral width number of points n**2. max_width chosen as 2**integer
	...
	for (int i=max_width; i>=1;i=i/2){
		...
		for (int k=1; k<=n+1; k++) {
			...
			/* #pragma omp parallel */
			{
			/* #pragma omp for nowait reduction(+:integral) */
			for (int l=1; l<=k; l++) {
				z = A*(l-1)/n;
		...
		if (n>=256 && (std::abs(std::abs((n*n*tmp)/(nPrev*nPrev*integral))-1.) <= error))
			break;
	}	
...
template <typename Pred, typename T>
VectorXd kspace_(T&& params, Pred predicate, int iterations, double abs){
